...
# Read config file
config <- read.ini('./Configs/lung_cancer_EMTmarkers.ini')
...
# After run a few rounds, hsa-miR-638 stands alone all of the time so
# we remove it in order to avoid 1-member groups
epi_marker_outlier <- which(epi_markers %in% c('hsa-miR-638'))
epi_markers <- epi_markers[-epi_marker_outlier]
...
```{r}
load(config$output$bothMarkersGroups)


# ==== 3.3.1 Mes Marker data matrix for the experiment ====
...
```{r}
pdag <- readRDS(config$output$bothMarkersVariableDag)
