...
Sys.setenv(JAVA_HOME='E:/Program Files (x86)/Java/bin')
...
```{r}
load(paste("./Data/BRCA_matchedData.RData", sep=""))
load(paste("./Data/BRCA_matchedData_normal_samples.RData", sep=""))

# miRNameList = colnames(matchedData$miRs)
# 
...
# }
# 
# miRNAVersionConvert(miRNANames = miRName)
# 
# colnames(matchedData$miRs) <- miRNameList

scan_df <- read.csv("./Data/TargetScan_v21.csv")
...
# diffexp_result <- DiffExp.limma(Tumor_Data = t(cancer.miRNAs), 
#                                  Normal_Data = t(normal.miRNAs), 
#                                  topk = 30)
# top30_miRNAs <- diffexp_result$`Subtype 1`$ID
...
# ==== 1.4 EMT =====
emt_df <- cbind(read.table("./Data/EMT.txt",stringsAsFactors=FALSE, header = FALSE),
                read.table("./Data/EMT_type.txt",stringsAsFactors=FALSE, header = FALSE))
...
group1 <- convert_to_col_numbers(miRNAs_groups, cancer_dm)
group2 <- convert_to_col_numbers(epi_groups, cancer_dm)
group3 <- convert_to_col_numbers(mes_groups, cancer_dm)

# foreach
split <- detectCores()
eachStart <- 25
iters = iter(rep(eachStart, split - 1))
